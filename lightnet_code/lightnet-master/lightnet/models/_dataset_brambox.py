...
class BramboxDataset(lnd.Dataset):
    ...
    def __getitem__(self, index):
        ...
        location = self.id(self.keys[index])
        location_depth = location[:-13]+"depth"+location[-10:]
        # print(location_depth)
        img_depth = Image.open(location_depth)
        ...
        if self.img_tf is not None:
            img = self.img_tf(img)
            img_depth = self.img_tf(img_depth)
            third_tensor = torch.cat((img, img_depth.float()), 0)
            # print(type(img),img.shape)
        if self.anno_tf is not None:
            anno = self.anno_tf(anno)

        # print(third_tensor.shape)
        return third_tensor,anno
        # return img, anno
